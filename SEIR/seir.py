...
monthly_baseline = {
    1: 3.2, 2: 3.0, 3: 2.5, 4: 2.0, 5: 1.8, 6: 1.6, 
    7: 1.4, 8: 1.5, 9: 1.8, 10: 2.2, 11: 2.8, 12: 3.2
}
...
def predict_ili_realistic(h3n2_pct, month):
    ...
    # 限制最低值，防止噪声导致负数或过低值
    return round(max(1.2, final_val), 2)

# 4. 执行填充
data['ili_pct_new'] = data['ili_pct'] # 保留原有的（最近19周）
empty_mask = data['ili_pct'].isna()

data.loc[empty_mask, 'ili_pct_new'] = data[empty_mask].apply(
    lambda row: predict_ili_realistic(row['h3n2_pct'], row['month']), axis=1
)

# 5. 保存
output_file = 'china_flu_realistic_final.csv'
data.drop(columns=['month']).to_csv(output_file, index=False, encoding='utf-8-sig')

print(f"✅ 还原完成！")
print(f"数据特征：月度基准控制在 {min(monthly_baseline.values())}% ~ {max(monthly_baseline.values())}%")
